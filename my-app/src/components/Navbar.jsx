    ...
    <div className="w-full bg-white shadow-md px-6 py-3 flex items-center justify-between relative">
      ...
      <div className="flex items-center space-x-6 relative">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-64 bg-white shadow-lg rounded-lg py-2 z-50">
              <p className="px-4 py-2 text-sm text-gray-400">You have {unreadCount} new messages</p>
            </div>
          )}
        </div>
        ...
        {user && (
          <div className="relative" ref={profileRef}>
            <button onClick={() => setShowProfile(!showProfile)} className="flex items-center space-x-2 hover:text-blue-600">
              <User size={22} className="text-gray-700" />
              <span className="hidden md:inline text-sm">{user.name}</span>
            </button>
            {showProfile && (
