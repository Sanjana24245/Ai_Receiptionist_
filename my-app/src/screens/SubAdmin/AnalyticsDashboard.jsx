...
const AnalyticsDashboard = () => {
  ...
  const aiAppointments = appointments.filter(appt => appt.type === "AI").length;
  const humanAppointments = appointments.filter(appt => appt.type === "human").length;
    ...
    <div className="flex h-screen">
      <div className="p-6 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-600">Live Count from DB</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">AI Handled</p>
                <p className="text-3xl font-bold text-gray-900">
                  {loading ? "..." : aiAppointments}
                </p>
                <p className="text-sm text-blue-600">AI Appointments</p>
              </div>
              <Bot className="w-8 h-8 text-green-500" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Human Handled</p>
                <p className="text-3xl font-bold text-gray-900">
                  {loading ? "..." : humanAppointments}
                </p>
                <p className="text-sm text-yellow-600">Human Appointments</p>
              </div>
              <UserCheck className="w-8 h-8 text-yellow-500" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Appointments</p>
                <p className="text-3xl font-bold text-gray-900">{loading ? "..." : totalAppointments}</p>
                <p className="text-sm text-purple-600">57% conversion rate</p>
              </div>
              <Calendar className="w-8 h-8 text-purple-500" />
            </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-blue-600">
                    {loading ? "..." : `${((aiAppointments / totalAppointments) * 100 || 0).toFixed(1)}%`}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className="bg-blue-500 h-2 rounded-full"
                    style={{ width: `${(aiAppointments / totalAppointments) * 100 || 0}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-green-600">
                    {loading ? "..." : `${((humanAppointments / totalAppointments) * 100 || 0).toFixed(1)}%`}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className="bg-green-500 h-2 rounded-full"
                    style={{ width: `${(humanAppointments / totalAppointments) * 100 || 0}%` }}
                  ></div>
                </div>
              </div>
        ...
        <div className="mt-8 bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">{loading ? "..." : totalAppointments}</div>
              <div className="text-sm text-gray-500">Total Interactions</div>
              <div className="text-xs text-green-500">↑ 18% vs last month</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{loading ? "..." : humanAppointments}</div>
              <div className="text-sm text-gray-500">Successful Appointments</div>
              <div className="text-xs text-green-500">↑ 24% vs last month</div>
            </div>
            <div className="text-center">
