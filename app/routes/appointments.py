...
async def create_appointment(appointment: Appointment):
    ...
    # If human appointment, check doctor leave
    if appointment.type == "human" and doctor.get("status") == "leave":
        raise HTTPException(
            status_code=400,
            detail="Doctor is on leave, cannot book human appointment"
        )

    # Round appointment time
    appt_time = round_to_nearest_half_hour(appointment.appointment_time)
    ...
    # Create patient_id automatically
    patient_id = str(ObjectId())  # generate a new ObjectId

    # Save appointment
    appt_dict = appointment.dict()
    appt_dict["doctor_id"] = str(appointment.doctor_id)
